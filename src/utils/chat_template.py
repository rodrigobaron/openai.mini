...
def build_chat_template(token_format_config):
    chat_template = "{% if not add_generation_prompt is defined %}{% seta add_generation_prompt = false %}{% endif %}{% for message in messages %}{%if message['role'] == 'system' %}{{'" + token_format_config.B_SYS + "' + message['content'] + '" + token_format_config.E_SYS + "'}}{% endif %}{%if message['role'] == 'user' %}{{'" + token_format_config.B_INST + "' + message['content'] + '" + token_format_config.E_INST + "'}}{% endif %}{%if message['role'] == 'assistant' %}{{'" + token_format_config.B_AI + "' + message['content'] + '" + token_format_config.E_AI + "'}}{% endif %}{% endfor %}{% if add_generation_prompt %}{{ '<|im_start|>assistant\n' }}{% endif %}"
